...
public class TranProducto extends JInternalFrame implements MouseListener, ActionListener, KeyListener {
	...
	public void actualizarProducto(int num,String cprod){
		
		int codi_prod=0;
		if(cprod.equals("0")){
			codi_prod=COD_PROD;
		}else{
			codi_prod=Integer.parseInt(cprod);
		}
	...
	public void compararProdPrecios(){
		...
		int codprove=0,codrubro=0,codmar=0,codumed=0;
		String modelo="",descripcion="",cod_prod="",cod_prod2="";
		int contInsert=0,contUpdate=0,conta=0;
			if(tam>0){
				for (int i = 0; i < objPrecios.tamaño(); i++) {
						...
						cod_prod=encontrarProdXModelo(modelo.trim()).trim();
						
						if(cod_prod.equals("")){
							agregarProducto(codrubro, descripcion, modelo);
							conta++;
							cod_prod2=encontrarProdXModelo(modelo).trim();
							contInsert++;
				...
				System.out.println("insert: "+contInsert+"  "+"update: "+contUpdate+"  "+"productos nuevos: "+conta);
				contInsert=0;
				contUpdate=0;
				conta=0;
			}
	...
	/***************************************************************************************************/
     public String  encontrarTransaccion(int prove, String prod, int mar, int umed){
     ...
     /***************************************************************************************************/
     public String  encontrarProdXModelo(String modelo){
		
		AccesoBD objAccesoBD=  new AccesoBD();
		objAccesoBD.crearConexion();
		 
		String sql="SELECT * FROM tb_producto "+
                    " where mod_prod='"+modelo+"' and est_prod='ACTIVADO' limit 1;";
   ...
   /**************************************************************************************************************/
     public void agregarPreciosxProductos(int prove, String prod, int mar, int umed,String cos_det,String mon_det,String igv_det){
    	 
 		AccesoBD objAccesoBD = new AccesoBD();
 		objAccesoBD.crearConexion();
 		     
 			String insertarPregunta="INSERT INTO tb_proveprodmarumed1(cod_prove, cod_prod, cod_mar, cod_umed, cos_det, mon_det, igv_det, obs_det, fec_det, est_det,est_rubro1)" +
 					" VALUES('"+prove+"','"+prod+"','"+mar+"','"+umed+"','"
   ...
   /***************************************************************************************************************/
     public void actualizarPreciosxProductos(int prove, String prod, int mar, int umed, String cos_det,String mon_det,String igv_det){
