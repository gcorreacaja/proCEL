...
public class BuscarProductoCotizacion extends JDialog implements ActionListener, MouseListener, KeyListener {
	...
	String titulo2[]={"COD","PRODUCTO","COSTE","CMAR","MARCA","MODELO","CUMED","UMED","FECHA","COD","PROVEEDOR","COD_PROD","NOM_PROD"};
		...
		try {
			COD_PROD=null;
				...
				this.setBounds(0, 0, 1054, 467);
					...
					pnlTransProducto.setBounds(556, 30, 434, 128);
					...
					}
					{
						...
						lblCodPromelsa.setBounds(193, 125, 86, 16);
					}
					{
						txtPromelsa = new JTextField();
						pnlBusProd.add(txtPromelsa);
						txtPromelsa.setBounds(274, 122, 110, 24);
					}
					{
	...
	public void buscarNormal(String nom){
		...
		"  	 WHERE PROD.NOM_PROD LIKE '"+NOMBREPROD+"%' AND MAR.NOM_MAR LIKE '%"+nommar+"%'" +
		...
		//int contCosas=0;
		//System.out.println("Busqueda Normal:"+sql);
	...
	/**************************************************************************************************************************************/
     ...
     public void buscar(){
       ...
       System.out.println("WWWWWWWW:"+act_prod);
