...
public class BuscarProducto extends JInternalFrame implements KeyListener, ActionListener, MouseListener, InternalFrameListener {
	...
	public BuscarProducto() {
		super("Buscar Producto", true, true, true, true);
		try {
				
			setVisible(true);
			this.setPreferredSize(new java.awt.Dimension(1310, 488));
			this.setBounds(0, 0, 1310, 488);
	...
	public void listar(){
	   ...
	   PintarFila();
		...
		String sql="SELECT DET.cod_proveprodmarumed,PROVE.COD_PROVE,PROVE.NOM_PROVE,PROD.COD_PROD,PROD.NOM_PROD,MAR.COD_MAR,MAR.NOM_MAR, "+
				   ...
				   "group by DET.cod_proveprodmarumed ORDER BY costoVenta asc limit 4000;";
	...
	public void buscarxRubro(){
		...
		String sql="SELECT DET.cod_proveprodmarumed,PROVE.COD_PROVE,PROVE.NOM_PROVE,PROD.COD_PROD,PROD.NOM_PROD,MAR.COD_MAR,MAR.NOM_MAR, "+
		   ...
		   "AND det.est_det='ACTIVADO' AND PROD.EST_PROD='ACTIVADO' AND RUB.EST_RUBRO='ACTIVADO' AND PROVE.EST_PROVE='ACTIVADO' "+ //and  DATE_SUB(CURDATE(),INTERVAL 90 DAY) <= DET.fec_det " +
		   "group by DET.cod_proveprodmarumed ORDER BY costoVenta asc;";
		System.out.println(sql);
		ResultSet rs = objAccesoBD.ejecutarConsulta(sql);
		...
		if(e.getSource()==jTable1){
			...
			if(evento.getSource()==jTable1){
				...
				if(evento.getClickCount()==1){
					...
					if(jTable1.getSelectedColumn()==13 && codi.length()>0){
						
						double porcAprox=0,
						       costo=Double.parseDouble(""+modelo2.getValueAt(fila, 7)),
						       precio=Double.parseDouble(""+modelo2.getValueAt(fila, 8));
						...
						objCal.txtCosto.setText(""+modelo2.getValueAt(fila, 7));
						objCal.txtDiferencia.setText(""+modelo2.getValueAt(fila, 13));
						objCal.txtPrecio.setText(""+modelo2.getValueAt(fila, 8));
				...
				if(evento.getClickCount()==2){
					nombreProducto=""+modelo2.getValueAt(fila,1);
